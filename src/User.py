...
def hands():
    ...
    logger.info("user_agents选择完毕")
    logger.info(f"USER_AGENTS:{Agents}")
    return Agents#在这里就把请求头选好
...
def hands2():
    while True:
        ...
        if 'Windows NT' in add_user:
            logger.info(f"add_user处理完毕+{add_user}")
            return add_user
        elif add_user.lower() == "q":
